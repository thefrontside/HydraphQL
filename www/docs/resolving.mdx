...
```typescript
const userDefinedResolver = field.resolve;
...
field.resolve = async (
  ...
  return userDefinedResolver(rawData, args, context, info);
}
```

With `@resolve` directive it generates
...
field.resolve = async (
  ...
  const { loader } = context;
  const node = await loader.load(source.id);
  const ref = _.get(node, directive.at);
...
Generated resolver for `@relation` directive is a special case of `@resolve` directive resolver.
But instead of taking `source` and value from `form` path we take entity ref from `relations` array
filtering by relation `name` and targetRef `kind`
